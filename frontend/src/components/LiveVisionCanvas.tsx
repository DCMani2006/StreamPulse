...
export const LiveVisionCanvas: React.FC<LiveVisionCanvasProps> = ({
  ...
  // Layer 1 (Bottom): Detection Bounding Boxes & Tracking
  ...
  // ---------------------------------------------------------------------------
  // Decoupled Mutable State Refs (Zero React Re-renders During Active Drag)
  // ---------------------------------------------------------------------------
  ...
  // React state strictly for static Toolbar UI sync (only updated on commit / button click)
  ...
  // Keep mutable telemetry refs in sync without triggering canvas redraw tears
  useEffect(() => {
  ...
  // Commit and emit ROI update to Backend REST/WebSocket
  ...
  // ---------------------------------------------------------------------------
  // Layer 1: YOLOv8 Detection Bounding Box Renderer
  // (Redrawn when new telemetry arrives)
  // ---------------------------------------------------------------------------
  useEffect(() => {
    ...
    if (telemetry?.detections && telemetry.detections.length > 0) {
      telemetry.detections.forEach((det) => {
        ...
        const insideRoi =
          isRoiActive && cx >= roi.x1 && cx <= roi.x2 && cy >= roi.y1 && cy <= roi.y2;

        if (insideRoi) {
          hasBreach = true;
        }

        const isProhibited = ['cell phone', 'laptop', 'knife', 'scissors', 'backpack'].includes(
          det.label.toLowerCase()
        );

        const isViolator = insideRoi || isProhibited || (det as any).is_violator;
        ...
        ctx.shadowBlur = isViolator ? 12 : 6;
        ...
        ctx.shadowBlur = 0;
        ctx.fillText(tagText, pillX + 6, pillY - 5);
        ctx.restore();
      });
    }
  ...
  // ---------------------------------------------------------------------------
  // Layer 2: 60 FPS Hardware-Accelerated Interactive Draggable/Resizable ROI Loop
  // (Zero React state updates during drag — buttery smooth rAF loop)
  // ---------------------------------------------------------------------------
  useEffect(() => {
    let animId: number;

    const renderInteractiveROI = () => {
      ...
      if (isEnabled) {
        ...
        if (isBreached) {
          // Breached Mode: Pulsing solid red with crimson fill
          ...
          ctx.beginPath();
          ctx.roundRect(rx1, Math.max(0, ry1 - 24), tw + 16, 22, [4, 4, 0, 0]);
          ctx.fill();
          ...
          // Normal Mode: Dashed cyan borders with high-contrast handles
          ...
          ctx.fillStyle = '#0891b2';
          ctx.beginPath();
          ctx.roundRect(rx1, Math.max(0, ry1 - 22), tw + 14, 20, [4, 4, 0, 0]);
          ctx.fill();

          ctx.fillStyle = '#ffffff';
          ctx.shadowBlur = 0;
          ctx.fillText(labelText, rx1 + 7, Math.max(14, ry1 - 7));
        }

        ctx.restore();
      }

      animId = requestAnimationFrame(renderInteractiveROI);
    };
  ...
  // ---------------------------------------------------------------------------
  // Native HTML5 Pointer Event Handlers (Hardware-Accelerated, Zero React Lag)
  // ---------------------------------------------------------------------------
  ...
  const hitTestHandle = (nx: number, ny: number): DragHandle => {
    ...
    const tol = 0.035; // Normalized hit threshold
  ...
  const handlePointerDown = (e: React.PointerEvent<HTMLCanvasElement>) => {
    ...
    if (handle) {
      ...
      // Native Pointer Capture guarantees smooth drag even when pointer leaves canvas bounds
      try {
  ...
  const handlePointerMove = (e: React.PointerEvent<HTMLCanvasElement>) => {
    ...
    // Hover state cursor updates (when not dragging)
    ...
    // Active Drag: Update coordinates directly in mutable ref (Zero React Lag!)
  ...
  const handlePointerUp = (e: React.PointerEvent<HTMLCanvasElement>) => {
    if (isDraggingRef.current) {
      ...
      // Single network commit on release (Zero network flooding during drag)
  ...
  return (
    <div className="relative w-full h-full">
      {/* Layer 1 (Bottom Canvas): Detection Bounding Boxes */}
      <canvas
